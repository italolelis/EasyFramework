        ...
        <section>
            ...
            <form id="formAgendamento" class="form has-validation">
                <div class="clearfix">
                    ...
                    <div class="form-input"><input type="date" id="data" name="data" class=":required" OnKeyPress="formatar('##/##/####', this)" maxLength="10" /></div>
                </div>
                <div class="clearfix">
                    <label for="hora" class="form-label">Hora do Agendamento <em>*</em><small>Digite uma hora válida</small></label>
                    <div class="form-input"><input type="text" id="hora" name="hora" class=":required" OnKeyPress="formatar('##:##:##', this)" maxLength="8"/></div>
                </div>
                <div class="clearfix">
                    ...
                    <div class="form-input">
                        <ul class="alternate">
                            {foreach $servicos as $servico}
                                <li><label for='{$servico->getId()}'>
                                        <input type="checkbox" name="servicos[]" id="{$servico->getId()}" value="{$servico->getNome()}" />
                                        {$servico->getNome()}</label>
                                </li>
                            {/foreach}
                        </ul>
                    </div>
...
{literal}
    ...
    {/literal}
              var dataString = $("input:text, select").serialize() + "&servicos=" + servicos  + "&cliente=" + {$sessao.id} + "";
    {literal}
              agendamento.add(dataString); 
