            ...
            <tbody>
                {foreach $lista_agendamento as $agendamento}
                    <tr class="{if $agendamento->getStatus() == 'AGENDADO'}gradeA{else}gradeX{/if}"> 
                        ...
                        <td class="center">{$agendamento->getCliente()}</td> 
                        <td class="center">{$agendamento->getDia()}</td> 
                        <td class="center">{$agendamento->getHora()}</td> 
                        <td class="center">{$agendamento->getStatus()}</td> 
                        <td class="center"><button onclick="location.href = '{$url.editarAgendamento}{$agendamento->getId()}'; return false;">Editar</button>|<button onclick="agendamento.deleteRegister('{$agendamento->getId()}')">Excluir</button></td> 
                    </tr>
                {/foreach}
            </tbody> 
        </table> 
