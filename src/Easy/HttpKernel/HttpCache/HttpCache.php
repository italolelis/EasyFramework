...
class HttpCache implements HttpKernelInterface, TerminableInterface
{
    ...
    public function handle(Request $request, $type = HttpKernelInterface::MASTER_REQUEST, $catch = true)
    {
        ...
        } elseif ($request->headers->contains('expect')) {
    ...
    protected function validate(Request $request, Response $entry, $catch = false)
    {
        ...
        $subRequest->headers->set('if_modified_since', $entry->headers->getItem('Last-Modified'));
        ...
        if (304 == $response->getStatusCode()) {
            ...
            foreach (array('Date', 'Expires', 'Cache-Control', 'ETag', 'Last-Modified') as $name) {
                if ($response->headers->contains($name)) {
                    $entry->headers->set($name, $response->headers->getItem($name));
                }
            }
    ...
    protected function fetch(Request $request, $catch = false)
    {
        ...
        if ($subRequest->headers->contains('if_modified_since')) {
            $subRequest->headers->remove('if_modified_since');
        }
        if ($subRequest->headers->contains('if_none_match')) {
    ...
    private function restoreResponseBody(Request $request, Response $response)
    {
        ...
        if ($response->headers->contains('X-Body-Eval')) {
            ob_start();

            if ($response->headers->contains('X-Body-File')) {
                include $response->headers->getItem('X-Body-File');
            } else {
        ...
        } elseif ($response->headers->contains('X-Body-File')) {
            $response->setContent(file_get_contents($response->headers->getItem('X-Body-File')));
        } else {
            return;
        }
    ...
    private function isPrivateRequest(Request $request)
    {
        foreach ($this->options['private_headers'] as $key) {
            ...
            if ('cookie' === $key) {
                if (count($request->cookies->GetArray())) {
                    return true;
                }
            } elseif ($request->headers->contains($key)) {
                return true;
            }
        }
