...
class Esi
{
    ...
    {
        if (null === $value = $request->headers->getItem('Surrogate-Capability')) {
            return false;
        }
    ...
    {
        $current = $request->headers->getItem('Surrogate-Capability');
        $new = 'easyFw="ESI/1.0"';
    ...
    {
        if (!$control = $response->headers->getItem('Surrogate-Control')) {
            return false;
        }
    ...
    public function process(Request $request, Response $response)
    {
        ...
        $type = $response->headers->getItem('Content-Type');
        if (empty($type)) {
            $type = 'text/html';
        }
        ...
        if ($response->headers->contains('Surrogate-Control')) {
            $value = $response->headers->getItem('Surrogate-Control');
