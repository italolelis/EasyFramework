...
class LogEngineCollection extends ObjectCollection
{
     ...
     * @throws LogException when logger class does not implement a write method
     */
    public function load($name, $options = array())
    {
        ...
        if (!$logger instanceof ILog) {
            throw new LogException(sprintf(
                            __d('easy_dev', 'logger class %s does not implement a write method.'), $loggerName
            ));
        }
    ...
    protected static function _getLogger($loggerName)
    {
        ...
        if (!class_exists($loggerName)) {
            throw new LogException(__d('easy_dev', 'Could not load class %s', $loggerName));
        }
        return $loggerName;
    }
