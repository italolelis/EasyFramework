...
App::uses('IGeneric', 'Generics');
App::uses('IEquatable', 'Generics');
        ...
        if (class_exists($name)) {
            ...
            if ($class->implementsInterface('IGeneric')) {
                ...
                if ($num != $genericTypesCount) {
                    throw new Exception("The generic type $name needs a different number of types ($num)!");
                }
            } else {
                if ($genericTypesCount != 0)
                    throw new Exception("The type $name is not generic!");
            }
        } else {
            if ($genericTypesCount != 0)
                throw new Exception("The type $name is not generic!");
