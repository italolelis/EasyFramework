...
class HttpResponse implements \ArrayAccess
{
    ...
    public function parseResponse($message)
    {
        if (!is_string($message)) {
            throw new Error\SocketException(__d('easy_dev', 'Invalid response.'));
        }

        if (!preg_match("/^(.+\r\n)(.*)(?<=\r\n)\r\n/Us", $message, $match)) {
            throw new Error\SocketException(__d('easy_dev', 'Invalid HTTP response.'));
        }
    ...
    protected function _decodeChunkedBody($body)
    {
        ...
        while ($chunkLength !== 0) {
            if (!preg_match('/^([0-9a-f]+) *(?:;(.+)=(.+))?(?:\r\n|\n)/iU', $body, $match)) {
                throw new Error\SocketException(__d('easy_dev', 'HttpSocket::_decodeChunkedBody - Could not parse malformed chunk.'));
            }
