...
class HttpSocket extends Socket
{
     ...
     * all other options:
     *
     * `$http = new HttpSocket('http://www.easyframework.net');`
     *
     * Or use an array to configure multiple options:
     *
     * {{{
     * $http = new HttpSocket(array(
     *    'host' => 'easyframework.net',
     *    'timeout' => 20
     * ));
     * }}}
     *
    ...
    public function setContentResource($resource)
    {
        ...
        if (!is_resource($resource)) {
            throw new Error\SocketException(__d('easy_dev', 'Invalid resource.'));
        }
    ...
    public function request($request = array())
    {
        ...
        if (!$responseClass) {
            throw new Error\SocketException(__d('easy_dev', 'Class %s not found.', $this->responseClass));
        }
     ...
     *
     * `$response = $http->get('http://google.com/search', array('q' => 'easyfw', 'client' => 'safari'));`
     *
     * Would do a GET request to `http://google.com/search?q=easyfw&client=safari`
     *
     ...
     * }}}
     *
     * Would return `http://www.easyframework.net/search?q=bar`
     *
     * url() can also be used with custom templates:
     *
     * `$url = $http->url('http://www.easyframework.net/search?q=socket', '/%path?%query');`
     *
    ...
    protected function _setAuth()
    {
        ...
        if (!$authClass) {
            throw new Error\SocketException(__d('easy_dev', 'Unknown authentication method.'));
        }
        if (!method_exists($authClass, 'authentication')) {
            throw new Error\SocketException(sprintf(__d('easy_dev', 'The %s do not support authentication.'), $authClass));
        }
    ...
    protected function _setProxy()
    {
        ...
        if (!$authClass) {
            throw new Error\SocketException(__d('easy_dev', 'Unknown authentication method for proxy.'));
        }
        if (!method_exists($authClass, 'proxyAuthentication')) {
            throw new Error\SocketException(sprintf(__d('easy_dev', 'The %s do not support proxy authentication.'), $authClass));
        }
    ...
    protected function _buildRequestLine($request = array(), $versionToken = 'HTTP/1.1')
    {
        ...
        if (is_string($request)) {
            ...
            if (!$this->quirksMode && (!$isValid || ($match[2] == '*' && !in_array($match[3], $asteriskMethods)))) {
                throw new Error\SocketException(__d('easy_dev', 'HttpSocket::_buildRequestLine - Passed an invalid request line string. Activate quirks mode to do this.'));
            }
            return $request;
        } elseif (!is_array($request)) {
            return false;
        ...
        if (!$this->quirksMode && $request['uri'] === '*' && !in_array($request['method'], $asteriskMethods)) {
            throw new Error\SocketException(__d('easy_dev', 'HttpSocket::_buildRequestLine - The "*" asterisk character is only allowed for the following methods: %s. Activate quirks mode to work outside of HTTP/1.1 specs.', implode(',', $asteriskMethods)));
        }
