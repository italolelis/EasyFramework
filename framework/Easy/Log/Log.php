 ...
 *
 * {{{
 * EasyLog::config('my_log', array('engine' => 'FileLog'));
 * }}}
 *
 ...
 *
 * {{{
 * EasyLog::error('Something horrible happened');
 * EasyLog::write(LOG_ERR, 'Something horrible happened');
 * }}}
 *
 ...
 *
 * See EasyLog::config() and EasyLog::write() for more information
 * on scopes
 *
 * @package       Easy.Log
 */
class Log
{
    ...
    public static function config($key, $config)
    {
        if (!preg_match('/^[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*/', $key)) {
            throw new LogException(__d('easy_dev', 'Invalid key name'));
        }
        if (empty($config['engine'])) {
            throw new LogException(__d('easy_dev', 'Missing logger classname'));
        }
    ...
    public static function enabled($streamName)
    {
        ...
        if (!isset(self::$_Collection->{$streamName})) {
            throw new LogException(__d('easy_dev', 'Stream %s not found', $streamName));
        }
    ...
    public static function write($type, $message, $scope = array())
    {
        ...
        $logged = false;
        foreach (self::$_Collection as $streamName) {
            $logger = $streamName;
            $types = null;
            $scopes = array();
